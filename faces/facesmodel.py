...
class FCResBlock(nn.Module):
    def __init__(self, dim, n, nonlinearity, batch_norm=False):
        """
...
class ObservationDecoder(nn.Module):
    ...
    def __init__(self, batch_size, sample_size, n_features,
        ...
        self.fc_initial = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.fc_linear = nn.Linear(self.hidden_dim, 16 * self.n_features)
        ...
        self.conv_final = nn.ConvTranspose2d(in_channels=3, out_channels=3,
                                             kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, zs, c):
        ...
        e = self.nonlinearity(e)
        e = self.fc_linear(e)
        e = e.view(-1, self.n_features, 4, 4)
...
class Statistician(nn.Module):
    ...
    def loss(self, outputs, weight):
        ...
        x, x_mean = x_outputs
        recon_loss = gaussian_log_likelihood(x.view(-1, 3, 64, 64), x_mean,
                                             Variable(torch.ones(x_mean.size()).cuda()))
